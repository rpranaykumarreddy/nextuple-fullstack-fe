...
export const useGetCashback = () => {
    ...
    const getCashback = async (month = new Date().getMonth() + 1, year = new Date().getFullYear()) => {
        if (!user || !user.sub) {
        ...
        setError(null);
        try {
            const response = await fetch(getCashbackAuthData.link + `?month=${month}&year=${year}`, {
            ...
            setLoading(false);
            setResponse(json);
            dispatch(showMessage({message: "Cashback fetched", severity: "success"}));
        } catch (error) {
