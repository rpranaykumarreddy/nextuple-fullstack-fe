...
export const checkUsernameAuthData = {
  link:
    GLOBALS.serverHost + GLOBALS.link.auth.id + GLOBALS.link.auth.checkUsername,
  method: "GET",
};
export const registerAuthData = {
  ...
  link: GLOBALS.serverHost + GLOBALS.link.auth.id + GLOBALS.link.auth.register,
  method: "POST",
};
export const useRegister = () => {
  ...
  const register = async () => {
    if (token) {
      setError("Already logged in. Clear cache");
      return;
    }
    setLoading(true);
    setError(null);
  ...
  const checkUsername = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${checkUsernameAuthData.link}/${data.username}`,
        {
